                    ...
                    <div class="col-md-8">
                        <select name="supplier_id" id="supplier_id" class="form-control">
                            ...
                            @foreach(app('App\Supplier')->orderBy('supplier_name')->get() as $model)
                                <option value="{{ $model->id }}"{{ $model->id == $stocks_transaction->supplier_id ? ' selected' : '' }}>{{ $model->supplier_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-8">
                        <select name="purchase_id" id="purchase_id" class="form-control">
                            ...
                            @foreach(app('App\Purchase')->orderBy('id')->get() as $model)
                                <option value="{{ $model->id }}"{{ $model->id == $stocks_transaction->purchase_id ? ' selected' : '' }}>{{ $model->id }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
