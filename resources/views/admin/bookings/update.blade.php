                    ...
                    <div class="col-md-8">
                        <select name="room_id" id="room_id" class="form-control">
                            ...
                            @foreach(app('App\Room')->orderBy('room_name')->get() as $model)
                                <option value="{{ $model->id }}"{{ $model->id == $booking->room_id ? ' selected' : '' }}>{{ $model->room_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-8">
                        <select name="guest_id" id="guest_id" class="form-control">
                            ...
                            @foreach(app('App\guest')->orderBy('last_name')->get() as $model)
                                <option value="{{ $model->id }}"{{ $model->id == $booking->guest_id ? ' selected' : '' }}>{{ $model->last_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
