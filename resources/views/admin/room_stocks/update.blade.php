...
@section('title', 'Update Room Stock')
@section('child-content')
    <h2>@yield('title')</h2>

    <form method="POST" action="{{ route('admin.room_stocks.update', $room_stock->id) }}" novalidate data-ajax-form>
        @csrf
        @method('PATCH')
        ...
        <div class="list-group">
                    ...
                    <div class="col-md-8">
                        <select name="room_id" id="room_id" class="form-control">
                            ...
                            @foreach(app('App\Room')->orderBy('room_name')->get() as $model)
                                <option value="{{ $model->id }}"{{ $model->id == $room_stock->room_id ? ' selected' : '' }}>{{ $model->room_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-8">
                        <select name="item_category_id" id="item_category_id" class="form-control">
                            ...
                            @foreach(app('App\ItemCategory')->orderBy('id')->get() as $model)
                                <option value="{{ $model->id }}"{{ $model->id == $room_stock->item_category_id ? ' selected' : '' }}>{{ $model->id }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="list-group-item">
                <div class="form-group row mb-0">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="stock_quantity" id="stock_quantity" class="form-control" value="{{ $room_stock->stock_quantity }}">
                    </div>
                </div>
            </div>
